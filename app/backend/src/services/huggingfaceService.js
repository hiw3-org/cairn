...
class HuggingFaceService {
  ...
  generateAuthUrl(userId) {
    ...
    const params = new URLSearchParams({
      ...
      scope: 'read-repos read-datasets profile',
      state: state
    });
    ...
    try {
      ...
      await User.findByIdAndUpdate(userId, {
        $set: {
          ...
          'integrations.huggingface.refreshToken': tokenData.refresh_token ? encrypt(tokenData.refresh_token) : user.integrations.huggingface.refreshToken,
          'integrations.huggingface.tokenExpiry': tokenExpiry,
          'integrations.huggingface.lastSync': new Date()
        }
      });
  ...
  async getUserRepos(userId, limit = 20) {
    try {
      ...
      const response = await fetch(`${this.apiUrl}/repos?limit=${limit}`, {
        headers: {
          'Authorization': `Bearer ${accessToken}`,
          'Accept': 'application/json'
        }
  ...
  async getUserDatasets(userId, limit = 20) {
    try {
      ...
      const response = await fetch(`${this.apiUrl}/datasets?author=me&limit=${limit}`, {
        headers: {
          'Authorization': `Bearer ${accessToken}`,
          'Accept': 'application/json'
        }
