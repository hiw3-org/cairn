...
const ProjectCard = ({ project, onSelectProject, onApplyClick, onSubmitForReproducibility }: { project: Project; onSelectProject: (p: Project) => void; onApplyClick: () => void; onSubmitForReproducibility: () => void; }) => {
    ...
    const renderAction = () => {
        switch(project.status) {
            case ProjectStatus.Draft:
                return <button onClick={onSubmitForReproducibility} disabled={project.outputs.length === 0} className="flex-1 bg-primary-light text-primary font-semibold py-2 px-3 rounded-lg hover:bg-blue-200 dark:bg-primary/20 dark:text-primary-light dark:hover:bg-primary/30 transition-all duration-300 text-sm disabled:opacity-50 disabled:cursor-not-allowed">Submit for Reproducibility</button>;
            case ProjectStatus.Reproducible:
                return <button onClick={onApplyClick} className="flex-1 bg-primary-light text-primary font-semibold py-2 px-3 rounded-lg hover:bg-blue-200 dark:bg-primary/20 dark:text-primary-light dark:hover:bg-primary/30 transition-all duration-300 text-sm">Apply to Funding</button>;
            case ProjectStatus.Funded:
                return <button className="flex-1 bg-primary-light text-primary font-semibold py-2 px-3 rounded-lg hover:bg-blue-200 dark:bg-primary/20 dark:text-primary-light dark:hover:bg-primary/30 transition-all duration-300 text-sm">View Round</button>;
            default:
                 return <button onClick={() => onSelectProject(project)} className="flex-1 bg-primary-light text-primary font-semibold py-2 px-3 rounded-lg hover:bg-blue-200 dark:bg-primary/20 dark:text-primary-light dark:hover:bg-primary/30 transition-all duration-300 text-sm">Manage</button>;
        }
    }
        ...
        <div 
            ...
            <div className="p-5 flex-grow">
                <div className="flex justify-between items-start mb-2">
                    <h3 className="text-lg font-semibold text-text-primary dark:text-dark-text-primary pr-2 group-hover:text-primary dark:group-hover:text-primary-light transition-colors">{project.title}</h3>
                ...
                <div className="mt-4 flex items-center space-x-6 text-sm">
                    <Tooltip text="Outputs Submitted"><div className="flex items-center space-x-2"><UploadCloudIcon className="w-5 h-5 text-text-secondary dark:text-dark-text-secondary"/><span className="font-semibold">{project.outputs.length}</span></div></Tooltip>
                    <Tooltip text="Verified PoRs"><div className="flex items-center space-x-2"><CheckCircleIcon className="w-5 h-5 text-text-secondary dark:text-dark-text-secondary"/><span className="font-semibold">{verifiedPors}</span></div></Tooltip>
                    <Tooltip text="Funding Raised"><div className="flex items-center space-x-2"><CurrencyDollarIcon className="w-5 h-5 text-text-secondary dark:text-dark-text-secondary"/><span className="font-semibold">{numberFormatter.format(project.fundingPool)}</span></div></Tooltip>
                </div>
            </div>
            
            <div className="border-t border-border dark:border-border-dark mt-auto p-3 flex items-center justify-between gap-2">
                 <button onClick={() => onSelectProject(project)} className="flex-1 bg-hf-gray-200 dark:bg-hf-gray-700 font-semibold py-2 px-3 rounded-lg hover:bg-hf-gray-300 dark:hover:bg-hf-gray-600 transition-all duration-300 text-sm">
                    View Project
                    </button>
                {renderAction()}
          </div>
...
const ProjectsView = ({ myProjects, onSelectProject, onNavigate, onNewProject, onApplyToFunding, onSubmitForReproducibility }: { myProjects: Project[], onSelectProject: (p: Project) => void, onNavigate: (page: string) => void, onNewProject: () => void, onApplyToFunding: (project: Project) => void, onSubmitForReproducibility: (projectId: string) => void }) => {
           ...
           <h1 className="text-3xl font-bold text-text-primary dark:text-dark-text-primary">Registered Projects</h1>
            ...
            {myProjects.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    {sortedProjects.map(p => <ProjectCard key={p.id} project={p} onSelectProject={onSelectProject} onApplyClick={() => onApplyToFunding(p)} onSubmitForReproducibility={() => onSubmitForReproducibility(p.id)} />)}
                        </div>
                      ) : (
  ...
  if (activePage === "funding") {
    ...
    content = (
      <div className="space-y-8">
        <PageHeader
          title="Discover & PoR"
          subtitle="Contribute to the community by reviewing projects and find new research."
        />
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <MyMetricsWidget
              ...
              totalPorsContributed={totalPorsContributed}
              porRequirement={porRequirement}
            />
            <DisputesWidget
              projects={projects}
              currentUser={currentUser}
              onViewContributionDetails={onViewContributionDetails}
            />
          </div>
        </div>
            ...
            {discoverProjects.length === 0 && (
              <div className="md:col-span-2 lg:col-span-3 xl:col-span-4 text-center py-16 px-6 border-2 border-dashed border-border dark:border-border-dark rounded-xl bg-cairn-gray-100 dark:bg-cairn-gray-800/50">
                ...
                <p className="mt-2 text-md text-text-secondary dark:text-text-dark-secondary">
                  Check back later to find new research to review and contribute
                  to.
                </p>
              </div>
            )}
      ...
      <>
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          <PageHeader
            ...
            subtitle="Manage your projects, track activity, and create new research initiatives."
          />
          <div className="relative group flex-shrink-0">
            ...
            {!isEligibleToCreate && (
              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-max max-w-xs text-center px-3 py-1.5 bg-text text-background-light dark:bg-text-dark dark:text-background-dark text-xs rounded-md shadow-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                {`Need ${
                  porRequirement - currentUser.porContributedCount
                } more PoRs to create a project (${porRequirement} total).`}
                <div className="absolute left-1/2 -translate-x-1/2 top-full w-0 h-0 border-x-4 border-x-transparent border-t-4 border-t-text dark:border-t-text-dark"></div>
              </div>
            )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          ...
          <div className="lg:col-span-1">
            <ActivityFeedWidget
              projects={projects}
              onViewReview={onViewContributionDetails}
            />
          </div>
          <div className="lg:col-span-1">
        ...
        <div>
          <h3 className="text-xl font-semibold text-text dark:text-text-dark mb-6">
            All My Projects
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            <NewProjectCard
