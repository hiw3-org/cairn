...
const ActionCenterWidget = ({
  ...
  onViewInfo: (round: FundingRound) => void;
  onMint: (roundId: string) => void;
}) => {
  ...
  const actions = useMemo(() => {
    const actionableNotifications = notifications
      .filter((n) =>
        ["deadline", "review_needed", "system_nudge"].includes(n.type)
      )
      .slice(0, 2)
    ...
    const mintableRounds = fundingRounds
      ...
      .map((r) => ({
        ...
        description:
          "Mint your Impact Asset NFT to commemorate your contribution.",
        action: { text: "Mint Impact Asset" },
        date: r.distributionDeadline,
        type: "mint" as const,
      }));

    return [...actionableNotifications, ...mintableRounds].sort(
  ...
  return (
    <div className="bg-background-light dark:bg-background-dark-light rounded-xl p-6 border border-border dark:border-border-dark shadow-sm h-full flex flex-col">
      <h3 className="font-semibold text-lg text-text-primary dark:text-dark-text-primary mb-4">
        Action Center
      </h3>
      {actions.length > 0 ? (
                ...
                <div className="flex-grow">
                  ...
                  {item.action && (
                    <button
                      ...
                      className="text-sm mt-1 font-semibold text-primary hover:underline"
                    >
                      {item.action.text}
                    </button>
                  )}
...
const ImpactAtAGlanceWidget = ({
  ...
  const {
    ...
    const fundedProjectIds = new Set(
      fundingRounds
        ...
        .flatMap((r) => r.applicants?.map((a) => a.projectId) || [])
    );

    const fundedProjects = projects.filter((p) => fundedProjectIds.has(p.id));

    const totalDownloads = fundedProjects.reduce(
      (sum, p) => sum + (p.adoptionMetrics?.huggingFaceDownloads?.value || 0),
      0
    );
    const totalCitations = fundedProjects.reduce(
      (sum, p) => sum + (p.scientificMetrics?.citations?.value || 0),
      0
    );
    const verifiedProjectsCount = fundedProjects.filter((p) =>
      p.reproducibilities.some((r) => r.status === PoRStatus.Success)
    ).length;
  ...
  const topProjects = [...fundedProjects]
    .sort(
      (a, b) =>
        (b.adoptionMetrics?.huggingFaceDownloads?.value || 0) -
        (a.adoptionMetrics?.huggingFaceDownloads?.value || 0)
    )
    .slice(0, 2);
  ...
  return (
    <div className="bg-background-light dark:bg-background-dark-light rounded-xl p-6 border border-border dark:border-border-dark shadow-sm h-full">
      ...
      <div className="grid grid-cols-3 gap-4 text-center">
        ...
        <div>
          <p className="text-2xl font-bold">
            {numberFormatter.format(totalCitations)}
          </p>
      ...
      <div className="mt-4 pt-4 border-t border-border dark:border-border-dark">
        ...
        <div className="space-y-2">
          {topProjects.map((p) => (
            <div
              key={p.id}
              className="text-sm p-2 rounded-md bg-hf-gray-100 dark:bg-hf-gray-800/50"
            >
              <p className="font-semibold truncate">{p.title}</p>
              <p className="text-xs text-text-secondary">
                {numberFormatter.format(
                  p.adoptionMetrics?.huggingFaceDownloads?.value || 0
                )}{" "}
                downloads
              </p>
            </div>
...
const DashboardView = ({
  ...
  const { totalDeployed, activeRounds, portfolioGrowth, aggregateImpactScore } =
    useMemo(() => {
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        ...
        subtitle="Track your portfolio, manage funding rounds, and view recent activity."
      />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <FunderKpiCard
          ...
          value={`$${numberFormatter.format(totalDeployed)}`}
        />
        <FunderKpiCard
          icon={GavelIcon}
          title="Active Rounds"
        ...
        <FunderKpiCard
          icon={TrendingUpIcon}
          title="Portfolio Growth (30d)"
          value={portfolioGrowth}
        />
        <FunderKpiCard
          icon={CheckBadgeIcon}
          title="Aggregate Impact Score"
          value={numberFormatter.format(aggregateImpactScore)}
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        <div className="lg:col-span-2 space-y-8">
          <ActionCenterWidget
            ...
            onMint={handleMintImpactAsset}
          />
        </div>
        <div className="lg:col-span-1 space-y-8">
          <ImpactAtAGlanceWidget
            projects={projects}
...
const PortfolioPerformanceWidget = ({
  ...
  const { fundingByDomain, impactPerK } = useMemo(() => {
    ...
    projects.forEach((p) => {
      if (p.domain in domainData) {
        domainData[p.domain] += p.fundingPool;
      }
    ...
    const fbd = Object.entries(domainData).map(([domain, amount]) => ({
      domain,
      amount,
      percentage: (amount / safeMaxFunding) * 100,
    }));

    const ipk =
      totalDeployed > 0 ? totalImpactScore / (totalDeployed / 1000) : 0;
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          {fundingByDomain.map((item) => (
            <div key={item.domain}>
              <div className="flex justify-between items-center text-sm mb-1">
                ...
                <span className="font-semibold text-text-secondary dark:text-dark-text-secondary">
                  ${numberFormatter.format(item.amount)}
                </span>
              </div>
              <div className="w-full bg-hf-gray-200 dark:bg-hf-gray-700 rounded-full h-2.5">
                <div
      ...
      <FunderKpiCard
        icon={TrendingUpIcon}
        title="Impact Score per $1K"
        value={impactPerK.toFixed(1)}
      />
    </div>
  );
};
...
const RoundCard = ({
  ...
  const getStatusConfig = () => {
    ...
    const configs = {
      ...
      Closed: {
        text: "Closed",
        color:
          "bg-hf-gray-200 text-hf-gray-600 dark:bg-hf-gray-700 dark:text-hf-gray-300",
      },
    };
  ...
  const fundedCount =
    round.applicants?.filter((a) => a.fundingAmount).length || 0;
    ...
    <div className="bg-background-light dark:bg-background-dark-light rounded-xl border border-border dark:border-border-dark p-4 flex flex-col justify-between">
      <div>
        <div className="flex justify-between items-start">
          <h4 className="font-bold text-text-primary dark:text-dark-text-primary pr-2">
            {round.title}
          </h4>
          <span
            className={`px-2 py-0.5 text-xs font-bold rounded-full whitespace-nowrap ${config.color}`}
          >
            {config.text}
          </span>
        </div>
        <div className="flex flex-wrap gap-1.5 mt-2">
          {round.topics.slice(0, 3).map((t) => (
            <span
              key={t}
        ...
        <div className="grid grid-cols-3 gap-4 text-center my-4">
          <div>
            <p className="font-bold text-lg">
              ${numberFormatter.format(round.poolSize)}
            </p>
            <p className="text-xs text-text-secondary">Pool Size</p>
          </div>
          <div>
            <p className="font-bold text-lg">{round.applicationCount}</p>
            <p className="text-xs text-text-secondary">Applicants</p>
          </div>
          <div>
            <p className="font-bold text-lg">{fundedCount}</p>
            <p className="text-xs text-text-secondary">Projects Funded</p>
          </div>
        </div>
      </div>
      <div className="mt-auto">
        {round.status === "Closed" && !round.impactAssetMinted && (
          <button
            onClick={handleMint}
            disabled={isMinting}
            className="w-full flex items-center justify-center space-x-2 bg-purple-600 text-white font-semibold py-2 px-3 rounded-lg hover:bg-purple-500 transition-colors disabled:bg-purple-400"
          >
            {isMinting ? (
...
const ImpactAssetCard = ({ round }: { round: FundingRound }) => {
  ...
  const { verifiedCount, totalCitations, totalDownloads } = useMemo(() => {
    const roundProjects = projects.filter((p) =>
      round.applicants?.some((a) => a.projectId === p.id)
    );
    const verified = roundProjects.filter((p) =>
      p.reproducibilities.some((r) => r.status === PoRStatus.Success)
    ).length;
    const citations = roundProjects.reduce(
      (sum, p) => sum + (p.scientificMetrics?.citations.value || 0),
      0
    );
    const downloads = roundProjects.reduce(
      (sum, p) => sum + (p.adoptionMetrics?.huggingFaceDownloads.value || 0),
      0
    );
    return {
    ...
    <div className="bg-background-light dark:bg-background-dark-light rounded-2xl border-2 border-border dark:border-border-dark shadow-lg overflow-hidden flex flex-col">
      ...
      <div className="p-4 flex-grow">
        ...
        <div className="text-xs text-text-secondary dark:text-dark-text-secondary pt-3 space-y-1">
          <p>
            <strong>Pool Size:</strong> ${round.poolSize.toLocaleString()}
          </p>
          <p>
            <strong>Domains:</strong> {round.topics.join(", ")}
          </p>
          <p>
            <strong>Minted On:</strong>{" "}
            {new Date(round.distributionDeadline).toLocaleDateString()}
          </p>
        </div>
      </div>
      <div className="p-3 bg-hf-gray-50 dark:bg-hf-gray-900/50 border-t border-border dark:border-border-dark flex items-center justify-center space-x-2">
        <button className="flex-1 flex items-center justify-center space-x-2 bg-primary text-white font-semibold text-sm py-2 px-3 rounded-lg hover:bg-primary-hover transition-colors">
          <ShareIcon className="w-4 h-4" />
          <span>Share</span>
        </button>
        <button className="flex-1 flex items-center justify-center space-x-2 bg-hf-gray-200 dark:bg-hf-gray-700 font-semibold text-sm py-2 px-3 rounded-lg hover:bg-hf-gray-300 dark:hover:bg-hf-gray-600 transition-colors">
...
const PortfolioView = ({ projects }: { projects: Project[] }) => {
  const { fundingHistory, fundingRounds, handleMintImpactAsset } =
    useAppContext();
  const [activeTab, setActiveTab] = useState<"Active" | "Voting" | "Closed">(
    "Active"
  );
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        title="Portfolio"
        subtitle="Analyze performance, track round history, and manage your Impact Certificates."
      />
      ...
      <div>
        ...
        <div className="border-b border-border dark:border-border-dark mb-4">
          <nav className="-mb-px flex space-x-6" aria-label="Tabs">
            {(["Active", "Voting", "Closed"] as const).map((tab) => (
              ...
              >
                {tab}{" "}
                <span className="bg-hf-gray-200 dark:bg-hf-gray-700 text-xs font-bold ml-2 px-2 py-0.5 rounded-full">
                  {roundsByStatus[tab].length}
                </span>
              </button>
            ))}
          </nav>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {roundsByStatus[activeTab].map((round) => (
            <RoundCard
              key={round.id}
              round={round}
              onMint={handleMintImpactAsset}
        ...
        {roundsByStatus[activeTab].length === 0 && (
      ...
      <div>
        ...
        {mintedAssets.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {mintedAssets.map((round) => (
              <ImpactAssetCard key={round.id} round={round} />
            ))}
          </div>
        ) : (
          <div className="text-center py-12 border-2 border-dashed border-border dark:border-border-dark rounded-xl">
            ...
            <h4 className="mt-4 text-xl font-semibold">
              No Impact Assets Minted Yet
            </h4>
            <p className="text-text-secondary dark:text-text-dark-secondary mt-2">
              When a round you participated in closes, you can mint an NFT here
              to certify your impact.
            </p>
          </div>
        )}
      </div>
