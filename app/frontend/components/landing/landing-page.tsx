...
import { BeakerIcon, ChartBarIcon, LinkIcon, SearchIcon, UsersGroupIcon, XIcon, GitHubIcon, IpfsIcon, WalletIcon, ArrowRightIcon, ChevronDownIcon, MenuIcon, CloseIcon, GavelIcon, GoogleIcon, SparklesIcon, MetaMaskIcon, OpenIdIcon, SpinnerIcon, HuggingFaceIcon, CheckCircleIcon } from '../ui/icons';
...
const AuthModal = ({ onClose, initialMode = 'login' }: { onClose: () => void; initialMode?: 'login' | 'signup' }) => {
    ...
    const renderSignupForm = () => (
         <form onSubmit={handleSignupSubmit} className="space-y-4">
             <p className="text-sm text-center text-text-secondary dark:text-text-dark-secondary">Submit your application to join the CAIRN platform. Your application will be manually reviewed.</p>
             <div>
                <label className="block text-sm font-medium text-text-secondary dark:text-text-dark-secondary mb-1">Full Name</label>
                <input type="text" value={signupName} onChange={e => setSignupName(e.target.value)} required className="w-full px-4 py-2 border border-border dark:border-border-dark rounded-lg bg-transparent focus:ring-1 focus:ring-primary focus:border-primary"/>
             </div>
             <div>
                <label className="block text-sm font-medium text-text-secondary dark:text-text-dark-secondary mb-1">I am a...</label>
                 <div className="flex gap-4">
                    <label className="flex-1 flex items-center p-3 border border-border dark:border-border-dark rounded-lg cursor-pointer has-[:checked]:bg-primary-light has-[:checked]:border-primary dark:has-[:checked]:bg-primary/20 dark:has-[:checked]:border-primary">
                        <input type="radio" name="role" value={UserRole.Researcher} checked={signupRole === UserRole.Researcher} onChange={() => setSignupRole(UserRole.Researcher)} className="h-4 w-4 text-primary focus:ring-primary border-cairn-gray-400"/>
                        <span className="ml-3 text-sm font-medium text-text dark:text-text-dark">Researcher</span>
                    </label>
                    <label className="flex-1 flex items-center p-3 border border-border dark:border-border-dark rounded-lg cursor-pointer has-[:checked]:bg-primary-light has-[:checked]:border-primary dark:has-[:checked]:bg-primary/20 dark:has-[:checked]:border-primary">
                        <input type="radio" name="role" value={UserRole.Funder} checked={signupRole === UserRole.Funder} onChange={() => setSignupRole(UserRole.Funder)} className="h-4 w-4 text-primary focus:ring-primary border-cairn-gray-400"/>
                        <span className="ml-3 text-sm font-medium text-text dark:text-text-dark">Funder</span>
                    </label>
                </div>
             </div>
             <div>
                <label className="block text-sm font-medium text-text-secondary dark:text-text-dark-secondary mb-1">Affiliation</label>
                <input type="text" placeholder="e.g., University of Cambridge" value={signupAffiliation} onChange={e => setSignupAffiliation(e.target.value)} required className="w-full px-4 py-2 border border-border dark:border-border-dark rounded-lg bg-transparent focus:ring-1 focus:ring-primary focus:border-primary"/>
             </div>
             <div>
                <label className="block text-sm font-medium text-text-secondary dark:text-text-dark-secondary mb-1">References (Optional)</label>
                <div className="space-y-2">
                     <input type="text" placeholder="GitHub Profile URL" value={signupGithub} onChange={e => setSignupGithub(e.target.value)} className="w-full px-4 py-2 border border-border dark:border-border-dark rounded-lg bg-transparent focus:ring-1 focus:ring-primary focus:border-primary"/>
                     <input type="text" placeholder="Google Scholar Profile URL" value={signupScholar} onChange={e => setSignupScholar(e.target.value)} className="w-full px-4 py-2 border border-border dark:border-border-dark rounded-lg bg-transparent focus:ring-1 focus:ring-primary focus:border-primary"/>
                     <input type="text" placeholder="LinkedIn Profile URL" value={signupLinkedin} onChange={e => setSignupLinkedin(e.target.value)} className="w-full px-4 py-2 border border-border dark:border-border-dark rounded-lg bg-transparent focus:ring-1 focus:ring-primary focus:border-primary"/>
                </div>
             </div>
             <button type="submit" disabled={isSubmitting} className="w-full flex items-center justify-center space-x-2 bg-blue-600 text-white font-semibold py-3 px-5 rounded-xl hover:bg-blue-500 transition-all duration-300 disabled:bg-blue-400">
                 {isSubmitting ? <SpinnerIcon className="animate-spin w-5 h-5"/> : <span>Submit Application</span>}
             </button>
    ...
    const renderLoginOptions = () => (
        <div className="space-y-6">
            <div>
                <h3 className="text-lg font-semibold text-center text-text dark:text-text-dark">Log in to your account</h3>
                <p className="text-sm text-center text-text-secondary dark:text-text-dark-secondary">Connect with your preferred account to continue.</p>
            </div>
            <div className="space-y-4">
                <button onClick={handleHuggingFaceLogin} className="w-full flex items-center space-x-4 p-4 border border-border dark:border-border-dark rounded-xl hover:bg-hf-gray-100 dark:hover:bg-hf-gray-800 transition-colors">
                    <HuggingFaceIcon className="w-8 h-8 text-yellow-500" />
                    <div className="text-left">
                        <p className="font-semibold text-text dark:text-text-dark">Continue as Researcher</p>
                        <p className="text-sm text-text-secondary dark:text-text-dark-secondary">Login with Hugging Face</p>
                    </div>
                </button>
                <button onClick={handleMetaMaskLogin} className="w-full flex items-center space-x-4 p-4 border border-border dark:border-border-dark rounded-xl hover:bg-hf-gray-100 dark:hover:bg-hf-gray-800 transition-colors">
                    <MetaMaskIcon className="w-8 h-8" />
                    <div className="text-left">
                        <p className="font-semibold text-text dark:text-text-dark">Continue as Funder</p>
                        <p className="text-sm text-text-secondary dark:text-text-dark-secondary">Login with MetaMask</p>
                    </div>
                </button>
            </div>
             <p className="text-center text-sm">
    ...
    const renderSignupSuccess = () => (
         <div className="text-center p-8">
            ...
            <h3 className="text-2xl font-bold mt-4 text-text dark:text-text-dark">Application Submitted!</h3>
            <p className="mt-2 text-text-secondary dark:text-text-dark-secondary">Thanks for applying. We'll review your application and get back to you soon.</p>
            <button onClick={onClose} className="mt-6 w-full bg-primary text-white font-semibold py-3 px-5 rounded-xl hover:bg-primary-hover">
                Close
            </button>
        </div>
    );
    ...
    return (
        <Modal onClose={onClose} title={mode === 'login' ? "Log In" : signupSuccess ? "Application Received" : "Apply to CAIRN"}>
            <div className="w-full max-w-md mx-auto">
                { signupSuccess ? renderSignupSuccess() : mode === 'login' ? renderLoginOptions() : renderSignupForm() }
            </div>
        </Modal>
    );
};
...
const LandingHeader = ({ onNavigate, onLoginClick, onSignupClick }: { onNavigate: (page: 'howitworks') => void; onLoginClick: () => void; onSignupClick: () => void; }) => {
    ...
    <header className="sticky top-0 z-40 bg-cairn-gray-950/80 backdrop-blur-lg border-b border-cairn-gray-800">
      <nav className="container mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-20">
        ...
        <div className="hidden md:flex items-center space-x-3">
             <button onClick={onLoginClick} className="font-semibold text-sm py-2.5 px-5 rounded-full hover:bg-cairn-gray-800 text-cairn-gray-200 hover:text-white transition-colors">Log In</button>
             <button onClick={onSignupClick} className="bg-blue-600 text-white font-semibold text-sm py-2.5 px-5 rounded-full hover:bg-blue-500 transition-all duration-300">
                Sign Up
             </button>
         </div>
              ...
              <div className="-my-6 divide-y divide-cairn-gray-700">
                <div className="space-y-2 py-6">
                 ...
                 <div className="py-6 space-y-4">
                    <button onClick={() => { onLoginClick(); setIsMobileMenuOpen(false); }} className="w-full flex items-center justify-center space-x-2 bg-cairn-gray-800 text-white font-semibold text-sm py-3 px-5 rounded-full">
                      <span>Log In</span>
                   </button>
                   <button onClick={() => { onSignupClick(); setIsMobileMenuOpen(false); }} className="w-full flex items-center justify-center space-x-2 bg-blue-600 text-white font-semibold text-sm py-3 px-5 rounded-full hover:bg-blue-500 transition-all duration-300 shadow-lg hover:shadow-blue-500/40">
                      <span>Sign Up</span>
                  </button>
                </div>
              </div>
...
const HeroSection = ({ onLoginClick, onSignupClick }: { onLoginClick: () => void; onSignupClick: () => void; }) => {
         ...
         <h1 className="text-5xl md:text-6xl lg:text-7xl font-extrabold tracking-tight" style={{ lineHeight: 1.1 }}>
          ...
          <p className="mt-6 max-w-2xl mx-auto text-lg md:text-xl text-cairn-gray-300">
            ...
            >
                             ...
                             <button onClick={enterAppAsGuest} className="font-semibold py-4 px-8 text-lg rounded-full border border-cairn-gray-600 hover:border-cairn-gray-400 hover:bg-cairn-gray-900 transition-all">
                                Explore Community Outputs
            </button>
                        </div>
                        <p className="pt-4 text-sm text-cairn-gray-400">
                            Don't have an account?{' '}
                            <button onClick={onSignupClick} className="font-semibold text-white hover:underline">
                                Apply to join
                            </button>
                        </p>
          </div>
...
export const LandingPage = ({ onNavigate }: { onNavigate: (page: 'howitworks') => void }) => {
  ...
  return (
    <div className="bg-cairn-gray-950 font-sans">
            <LandingHeader onNavigate={onNavigate} onLoginClick={() => openAuthModal('login')} onSignupClick={() => openAuthModal('signup')} />
      <main>
          <HeroSection 
      ...
      <AppFooter />
       {isAuthModalOpen && <AuthModal onClose={() => setIsAuthModalOpen(false)} initialMode={authModalMode} />}
    </div>
  );
};
