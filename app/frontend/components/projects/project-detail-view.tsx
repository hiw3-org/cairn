  ...
  return (
        <div className="bg-background-light dark:bg-background-dark-light rounded-xl shadow-sm p-6 border border-border dark:border-border-dark">
            <div className="space-y-6">
                {/* Funding Section */}
                <div>
                    <h3 className="text-lg font-semibold text-text dark:text-text-dark mb-3">Funding</h3>
                ...
                {project.impactAssetOwners && project.impactAssetOwners.length > 0 && (
                    <div>
                        <h3 className="text-lg font-semibold text-text dark:text-text-dark mb-2">Ownership</h3>
                        <ul className="space-y-2">
                            {project.impactAssetOwners.map((owner, index) => (
                                <li key={index} className="py-2 flex justify-between items-center">
                                    <div>
                                        <p className="font-semibold text-text dark:text-text-dark text-sm">{owner.contribution}</p>
...
const OutputCard = ({ output, projectStatus }: { output: Output, projectStatus: ProjectStatus }) => {
  ...
  return (
        ...
        <div>
                <span className="text-xs font-semibold bg-cairn-gray-200 dark:bg-cairn-gray-700 px-2 py-0.5 rounded-full">{output.type}</span>
                <p className="font-semibold text-text dark:text-text-dark mt-2">{output.description}</p>
                <p className="text-xs text-text-secondary dark:text-text-dark-secondary">Added on {output.timestamp}</p>
        </div>
          ...
          <div className="flex justify-between items-start">
            <div>
                                <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mb-2">
                                    ...
                                    <span className="text-sm text-text-secondary dark:text-text-dark-secondary">Last updated: {project.lastOutputDate}</span>
                </div>
                                <h1 className="text-3xl font-bold text-text dark:text-text-dark">{project.title}</h1>
                                <div className="flex flex-wrap gap-1.5 mt-2">
                                    {project.tags.map(tag => (
                                        <span key={tag} className="text-xs bg-cairn-gray-200 dark:bg-cairn-gray-700 px-2 py-0.5 rounded-full">{tag}</span>
                                    ))}
            </div>
          </div>
                             <div className="flex items-center space-x-2 flex-shrink-0 ml-4">
                                 <button onClick={() => setIsStarred(s => !s)} className={`p-2 rounded-full transition-colors ${isStarred ? 'text-yellow-500 bg-yellow-100 dark:bg-yellow-500/20' : 'hover:bg-cairn-gray-200 dark:hover:bg-cairn-gray-800'}`}>
                                     {isStarred ? <StarFilledIcon className="w-5 h-5" /> : <StarIcon className="w-5 h-5" />}
                                 </button>
                                 <button className="p-2 rounded-full hover:bg-cairn-gray-200 dark:hover:bg-cairn-gray-800"><ShareIcon className="w-5 h-5" /></button>
          </div>
            </div>

                        <p className="mt-4 text-text-secondary dark:text-text-dark-secondary">{project.description}</p>
                    </div>
