      ...
      <div className="flex items-start space-x-4">
        ...
        <div className="flex-grow">
          ...
          <div className="mt-4 space-y-2">
            ...
            {output.data.url && (
              <DataBox>
                {isGitUrl(output.data.url) ? (
                  <GitHubIcon className="w-5 h-5 text-text-secondary flex-shrink-0" />
                ) : (
                  <LinkIcon className="w-5 h-5 text-text-secondary flex-shrink-0" />
                )}
                <a
                  href={
                    !output.data.url.startsWith("http")
                      ? `https://${output.data.url}`
                      : output.data.url
                  }
                  target="_blank"
            ...
            {output.data.ipfsCid && (
              <DataBox>
                ...
                <button
                  onClick={() => copy(output.data.ipfsCid!)}
                  className="p-1 rounded hover:bg-cairn-gray-300 dark:hover:bg-cairn-gray-600"
                >
                  {copied ? (
                    <CheckIcon className="w-4 h-4 text-status-success" />
                ...
                <a
                  href={`https://ipfs.io/ipfs/${output.data.ipfsCid}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="p-1 rounded hover:bg-cairn-gray-300 dark:hover:bg-cairn-gray-600"
                >
            ...
            {output.data.otherText && (
              <DataBox>
                <span className="text-text dark:text-text-dark whitespace-pre-wrap">
                  {output.data.otherText}
                </span>
              </DataBox>
            )}
            ...
            {output.data.fileName && (
              <DataBox>
                ...
                <span className="text-text dark:text-text-dark font-medium flex-grow">
                  {output.data.fileName}
                </span>
                {!output.data.ipfsCid && (
