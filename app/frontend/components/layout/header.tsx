...
const IconButton = ({
  ...
  <button
    onClick={onClick}
    className={`p-2 rounded-full text-text-secondary dark:text-dark-text-secondary hover:bg-hf-gray-100 hover:text-text-primary dark:hover:bg-hf-gray-800 dark:hover:text-dark-text-primary transition-colors ${className}`}
  >
    {children}
  </button>
);
  ...
  return (
    <div className="relative" ref={menuRef}>
      <button
        ...
        className="flex items-center space-x-2 p-2 rounded-full hover:bg-hf-gray-100 dark:hover:bg-hf-gray-800 transition-colors"
      >
        <UserCircleIcon className="w-8 h-8 text-text-secondary dark:text-dark-text-secondary" />
        <div className="hidden md:flex flex-col items-start">
          <span className="text-sm font-semibold text-text-primary dark:text-dark-text-primary font-mono">
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-64 bg-background-light dark:bg-background-dark-light rounded-xl shadow-2xl border border-border dark:border-border-dark z-50 py-2 animate-modal-scale-in origin-top-right">
          <div className="px-4 py-2 border-b border-border dark:border-border-dark mb-2">
            ...
            <p
              className="text-sm text-text-secondary dark:text-dark-text-secondary truncate"
              title={currentUser.walletAddress}
            >
              <WalletIcon className="inline w-3 h-3 mr-1" />
          ...
          <button
            ...
            className="w-full text-left px-4 py-2 text-sm text-text-primary dark:text-dark-text-primary hover:bg-hf-gray-100 dark:hover:bg-hf-gray-800 transition-colors"
          >
            Disconnect
          </button>
        </div>
